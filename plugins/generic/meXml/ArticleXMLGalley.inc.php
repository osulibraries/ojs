...
class ArticleXMLGalley extends ArticleHTMLGalley {
	...
	function _xsltCacheMiss(&$cache) {
		...
		if (!isset($contents)) {
            ...
            error_log("xsltRenderer:[".$xsltRenderer."]");
	...
	function getHTMLContents() {
        ...
        error_log("getFileContents complete");
		return $contents;
	}
		...
		if ( !FileManager::fileExists($pdfFileName) || filemtime($pdfFileName) < filemtime($this->getFilePath()) ) {
			...
			if ($status != false) {
				...
				if ($contents != '') {
                    ...
                    error_log("No FOP error message");
                    return false;
                }
			}
	...
	{
		error_log("Override - getImageFiles");
		if ($this -> isHTMLGalley())
		{
			...
			if ($assocArticle != null) {
                error_log("AssocArticle not null");
	...
	function transformXSLT($xmlFile, $xslFile, $xsltType = "", $arguments = null) {
		...
		error_log("Found files: [XML, XSL]");

		// Determine the appropriate XSLT processor for the system
        $myPHPVERSION = PHP_VERSION;
        $myXSL = extension_loaded('xsl');
        $myDOM = extension_loaded('dom');

		if ( version_compare(PHP_VERSION,'5','>=') && extension_loaded('xsl') && extension_loaded('dom') ) {
			error_log("PHP5.x with XSL/DOM modules present");

            error_log("xsltType:[".$xsltType."]");
			if ( $xsltType == "PHP5"  || $xsltType == "" ) {
                error_log("load the XML file as a domdocument");
		...
		if ( version_compare(PHP_VERSION,'5','<') && extension_loaded('xslt') ) {
			error_log("PHP4.x with XSLT module present");
		...
		if ( $xsltType == "saxon" ) {
			error_log("PDF transform using java, saxon and XSLT 2.0");
            ...
            error_log("FO Command: " . $xsltCommand);
