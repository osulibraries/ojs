...
class ArticleXMLGalleyDAO extends ArticleGalleyDAO {
	...
	function _getXMLGalleyFromId($galleyId, $articleId = null) {
		...
		if (isset($articleId)) {
            ...
            //Changed query from the limited selectors below, and instead
            /*x.*,
	...
	function appendXMLGalleys($hookName, $args) {
		...
		while (list($key) = each($galleys)) {
			...
			if ($galley->getFileType() == "text/xml") {
                ...
                //Uh-oh, we don't want this plugin to be powered by pre-populating, we want it to on-the-fly determine our
	...
	function insertXMLGalleys($hookName, $args) {
        ...
        //If the default label is called Untitled, then not XML treatment
		if ($galley->getLabel() == "XML" || $_FILES['layoutFile']['type'] == 'text/xml') {
    ...
    function updateXMLGalleys($hookName, $args) {
        error_log("Recreating an XML Galley");
	...
	function deleteXMLGalleys($hookName, $args) {
		$galleyId =& $args[0];
		$articleId =& $args[1];
        error_log("deleteXMLGalleys called on articleID:" . $articleId);
